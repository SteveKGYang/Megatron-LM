...
# from megatron.utils import get_ltor_masks_and_position_ids, is_mp_rank_0 #mpu.get_tensor_model_parallel_rank() == 0
...
def stream_tokens(
    ...
    context_tokens, attention_mask, position_ids = get_batch(tokenizer, context_tokens) #look into
    ...
    with torch.no_grad():
        ...
        generation_logits = (
            torch.empty(maximum_tokens, tokenizer.vocab_size).float().cuda()
        )
